...
const getAnimalsNames = (animal, animalSex) => {
  ...
  if (animalSex) {
    animalNames = residents.filter((resident) => resident.sex === animalSex).map((residentName) => residentName.name);
...
const getListWithNamesSorted = (param1, paramSex) => {
  // console.log('PARAM SEX PRIMEIRA FUNÇÃO', paramSex)
  const sexOption = paramSex;
  console.log('SEX DENTRO DA CONST FUNÇÃO 2', sexOption)
  const allLocations =  getAllLocations();
  const { NE, NW, SE, SW } = allLocations;
